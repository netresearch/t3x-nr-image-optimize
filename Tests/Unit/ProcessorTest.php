...
final class ProcessorTest extends TestCase
{
    ...
    {
        $scenario = $this->setUpProcessAndRespondScenario('nr-pio-avif-cap-', 'jpg', 400, 200, 200, 'skipWebP=1');
        $this->injectExtensionConfiguration($scenario['processor'], ['qualityAvif' => '100']);

        /** @var ArrayObject<string, mixed> $saved path => named options of every save() call */
        $saved = new ArrayObject();
        $image = $scenario['image'];
        $image->expects(self::exactly(2))->method('save')->willReturnCallback(
            static function (?string $path = null, mixed ...$options) use ($saved, $image): ImageInterface {
                $saved[(string) $path] = $options;
        ...
        self::assertSame($scenario['response'], $this->invokeProcessAndRespond($scenario));
        self::assertSame(['quality' => 99], $saved[$scenario['variantPath'] . '.avif']);
    ...
    // =========================================================================
    // Animated GIF detection + passthrough (issue #142)
    // =========================================================================

    /**
     * Build a minimal, valid, looping two-frame 1x1 animated GIF.
     *
     * Generated byte-by-byte so the fixture is deterministic and needs
     * neither GD nor Imagick: header + logical screen descriptor with a
     * two-color global color table, NETSCAPE looping extension, and two
     * frames each consisting of a Graphic Control Extension, an image
     * descriptor, and minimal LZW image data.
     */
    private function buildAnimatedGifBytes(): string
    {
        $frame = "\x21\xF9\x04\x04\x0A\x00\x00\x00"          // GCE: delay 10, no transparency
            . "\x2C\x00\x00\x00\x00\x01\x00\x01\x00\x00"     // Image descriptor 1x1, no LCT
            . "\x02\x02\x44\x01\x00";                        // LZW min code size 2, data, terminator

        return 'GIF89a'
            . "\x01\x00\x01\x00\x80\x00\x00"                 // Screen 1x1, GCT (2 colors)
            . "\x00\x00\x00\xFF\xFF\xFF"                     // GCT: black, white
    ...
    /**
     * Build a minimal, valid, single-frame 1x1 static GIF (with a Graphic
     * Control Extension, as GIFs saved with transparency carry one too).
     */
    private function buildStaticGifBytes(): string
    {
        return 'GIF89a'
    ...
    /**
     * Write GIF bytes to a fresh temporary file and return its path.
     */
    private function createTempGifFile(string $bytes): string
    {
        $path = sys_get_temp_dir() . '/nr-pio-gif-' . uniqid('', true) . '.gif';
    ...
    #[Test]
    public function isAnimatedGifDetectsMultiFrameGif(): void
    {
        $path = $this->createTempGifFile($this->buildAnimatedGifBytes());

        try {
            self::assertTrue($this->callMethod($this->processor, 'isAnimatedGif', $path));
        } finally {
    ...
    #[Test]
    public function isAnimatedGifReturnsFalseForSingleFrameGif(): void
    {
        $path = $this->createTempGifFile($this->buildStaticGifBytes());

        try {
            self::assertFalse($this->callMethod($this->processor, 'isAnimatedGif', $path));
        } finally {
            unlink($path); // nosemgrep: php.lang.security.unlink-use.unlink-use -- test fixture teardown of self-created tmp file
        }
    }
    ...
    #[Test]
    public function isAnimatedGifReturnsFalseForNonGifContent(): void
    {
        $path = $this->createTempGifFile('this is not a gif at all');
    ...
    #[Test]
    public function isAnimatedGifReturnsFalseForMissingFile(): void
    {
        self::assertFalse(
            $this->callMethod($this->processor, 'isAnimatedGif', sys_get_temp_dir() . '/nr-pio-does-not-exist.gif'),
        );
    }

    /**
     * The scanner reads the file in 100 KiB chunks and carries the last
     * 9 bytes over to the next chunk. Place the second frame separator
     * exactly across the first chunk boundary to prove a boundary-spanning
     * separator is still counted (the detector is a byte scanner, so the
     * inert padding bytes between the frames do not need to parse as GIF
     * blocks).
     */
    #[Test]
    public function isAnimatedGifDetectsFrameSeparatorSpanningChunkBoundary(): void
    {
        $chunkSize = 102_400;
        ...
        // The 10-byte separator ("\x00" block terminator + GCE + "\x2C")
        // must start 5 bytes before the chunk boundary. Chunks are read
        // after the 6-byte header probe, so the first boundary sits at
        // absolute file offset 6 + $chunkSize.
        $separatorStart = 6 + $chunkSize - 5;
        $padding        = str_repeat("\xAA", $separatorStart - strlen($prefix));

        $bytes = $prefix
            . $padding
            . "\x00\x21\xF9\x04\x04\x0A\x00\x00\x00\x2C"
            . "\x00\x00\x00\x00\x01\x00\x01\x00\x00"
            . "\x02\x02\x44\x01\x00"
            . "\x3B";

        $path = $this->createTempGifFile($bytes);
    ...
    #[Test]
    public function processAndRespondPassesAnimatedGifThroughUnprocessed(): void
    {
        $scenario = $this->setUpProcessAndRespondScenario('nr-pio-anim-gif-', 'gif', 795, 535, 200, '');

        $animatedBytes = $this->buildAnimatedGifBytes();
        file_put_contents($scenario['originalPath'], $animatedBytes);

        // Passthrough must never touch the image pipeline: no decode-based
        // dimension math, no resize, no encode of any variant format.
        $scenario['image']->expects(self::never())->method('save');
        $scenario['image']->expects(self::never())->method('cover');
        $scenario['image']->expects(self::never())->method('scale');

        self::assertSame($scenario['response'], $this->invokeProcessAndRespond($scenario));

        // The original bytes are cached at the variant path so follow-up
        // requests are served straight from disk by serveCachedVariant().
        self::assertFileExists($scenario['variantPath']);
        self::assertSame($animatedBytes, (string) file_get_contents($scenario['variantPath']));
        self::assertFileDoesNotExist($scenario['variantPath'] . '.webp');
        self::assertFileDoesNotExist($scenario['variantPath'] . '.avif');

        $this->tearDownProcessAndRespondScenario($scenario['tempDir']);
    }

    #[Test]
    public function processAndRespondStillProcessesStaticGif(): void
    {
        $scenario = $this->setUpProcessAndRespondScenario('nr-pio-static-gif-', 'gif', 400, 200, 200, '');

        file_put_contents($scenario['originalPath'], $this->buildStaticGifBytes());

        // A single-frame GIF keeps the normal pipeline: primary + WebP + AVIF.
        $capture = $this->captureSaveCall($scenario['image'], 'processed', expectedCallCount: null);
