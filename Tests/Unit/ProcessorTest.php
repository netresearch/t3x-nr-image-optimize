...
class ProcessorTest extends TestCase
{
    ...
    // =========================================================================
    // Animated GIF detection + passthrough (port of upstream #143 / issue #142)
    // =========================================================================
    ...
    {
        $scenario = $this->setUpProcessAndRespondScenario('nr-pio-anim-gif-', 'gif', 795, 535, 200, 0, '');

        $animatedBytes = $this->buildAnimatedGifBytes();
        file_put_contents($scenario['originalPath'], $animatedBytes);

        // Passthrough must never touch the encoder paths of the image
        // pipeline. save()/cover()/scale() are already stubbed by the
        // scenario helper, so their non-invocation is asserted via the
        // capture holder below instead of an expects(never()) that a
        // previously registered stub matcher could shadow.
        $scenario['image']->expects(self::never())->method('toWebp');
        $scenario['image']->expects(self::never())->method('toAvif');
        ...
        // save() was never called: the capture holder still has no path, and
        // the variant file holds the untouched original bytes rather than
        // the 'processed-image' payload the save() stub would have written.
        self::assertNull($scenario['capture']['path']);
    ...
    {
        $scenario = $this->setUpProcessAndRespondScenario('nr-pio-static-gif-', 'gif', 400, 200, 200, 0, '');
        ...
        self::assertSame($scenario['variantPath'], $scenario['capture']['path']);
